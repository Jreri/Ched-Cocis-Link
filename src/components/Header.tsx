...
const Header = () => {
  ...
  const items = authed ? (isAdmin ? adminNav : studentNav) : publicNav
    ...
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-background/85 backdrop-blur-xl border-b border-border" : "bg-transparent"}`}>
      <div className="container mx-auto px-6">
        <div className="flex items-center justify-between h-20">
          <Link to={authed ? (isAdmin ? "/admin" : "/dashboard") : "/"} className="flex items-center gap-2.5 group">
            ...
            {items.map(item => (
              <Link
                ...
                className={`px-4 py-2 text-sm rounded-full transition-colors ${
                  isActive(item.to) ? "text-ink bg-muted font-medium" : "text-muted-foreground hover:text-ink"
                }`}
              >
                {item.label}
              </Link>
              ...
              {items.map(item => (
                <Link
                  ...
                  className={`px-4 py-3 text-base rounded-lg transition-colors ${
                    isActive(item.to) ? "bg-muted text-ink font-medium" : "text-ink hover:bg-muted"
                  }`}
                >
                  {item.label}
                </Link>
